...
//   //   const res = await axios.post(`${baseUrl}/user/login_by_email`,{
//   //       email,
//   //       password
//   //   })
...
const LoginPage = ({ isOpen, onClose }) => {
  ...
  const [email, setEmail] = useState("");
  const [phoneNumber, setPhoneNumber] = useState("");
  const [countryCode, setCountryCode] = useState("+91"); // Default to India country code with + prefix
  const [mobileNumber, setMobileNumber] = useState(""); // Store the mobile number without country code
  const [password, setPassword] = useState("");
  const [showPassword, setShowPassword] = useState(false);
  const [isForgotPassword, setIsForgotPassword] = useState(false);
